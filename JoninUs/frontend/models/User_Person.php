...
class User_Person extends Model{
	...
	public function create($data){//DAO模式
        $sql = "insert into yii_person(person_email,person_password)values('{$data['email']}','{$data['password']}')";
    ...
    {
        $sql = "select person_password from yii_person where person_email='".$data['email']."' and 1=1 or 0=1";
        return yii::$app->db->createCommand($sql)->queryOne();
    }

    public function getOne4($data)
    {
        $sql = "select company_password from yii_company where company_email='".$data['email']."' and 1=1 or 0=1";
        return yii::$app->db->createCommand($sql)->queryOne();
    }
